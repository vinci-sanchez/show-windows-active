...
async function send_data(){
    ...
    if(active.exe === 'Tai.exe'){return;};
    const mem = await si.mem();
    const mem_str = `${(mem.used / 1024 / 1024 / 1024).toFixed(2)} GB / ${((mem.used / mem.total) * 100).toFixed(2)}%`;
    const now = new Date();
    const time_str = now.toTimeString().split(' ')[0]; // "HH:MM:SS"
    try{
